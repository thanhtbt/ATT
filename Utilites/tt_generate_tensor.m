function [X,tt_core] = tt_generate_tensor(tt_dim,tt_rank,epsilon)
...
for ii = 1:T    
    ...
    Xii   = Xii(:,:,:,1);
    X(:,:,:,ii) = Xii.data;
    if epsilon(ii) == 1 %% Create a significant change
          G1    = randn(size(G1));
          G2    = randn(size(G2));
          G3    = randn(size(G3));
    else
          G1    = G1  + epsilon(ii)*randn(size(G1));
          G2    = G2  + epsilon(ii)*randn(size(G2));
          G3    = G3  + epsilon(ii)*randn(size(G3));
    end
